...
class acc_common_vseq extends acc_base_vseq;
  ...
  virtual task sec_cm_inject_fault(sec_cm_base_if_proxy if_proxy);
    // When testing the countermeasures for the ACC hardware loop stack, the current_loop_valid
    ...
    // `current_loop.loop_addr_info.loop_start` via `insn_fetch_req_addr_o`). But ACC will
